    ...
    <div className='min-h-screen bg-gradient-to-b from-slate-50 to-white'>
      <div className='max-w-6xl  mx-auto p-6'>
        ...
        <div className='flex items-center gap-3'>
          <img src={userData.profile_picture} alt="" className='w-12 h-12 rounded-full shadow' />
          <div>
            <h2 className='font-semibold'>{userData.full_name}</h2>
            <p className='text-sm text-gray-500'>@{userData
              .username}</p>
          </div>
        </div>
...
<textarea className='w-full resize-none max-h-20 mt-4 text-sm outline-none placeholder-gray-400' placeholder="What's happening?" onChange={(e)=>setContent(e.target.value)} value={content}/>
 ...
 } className='text-sm bg-gradient-to-r from-indigo-600 to-blue-600  hover:from-blue-600 hover:to-indigo-600  active:scale-95 transition text-white font-medium px-8 py-2 rounded-md cursor-pointer'>Publish Post</button>
 </div>
